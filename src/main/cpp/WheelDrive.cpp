...
WheelDrive::WheelDrive(int angleMotorPort, int speedMotorPort, int encoder)
    : angleMotor(angleMotorPort, "Drivebase"),
      speedMotor(speedMotorPort, "Drivebase"),
    ...
    // m_canCoder.SetStatusFramePeriod(CANCoderStatusFrame::CANCoderStatusFrame_SensorData,
    // 20, 100);
    m_canCoder.ClearStickyFaults();
}

// Drive function, set each module's speed and angle
void WheelDrive::drive(double speed, double angle, double offSet) {
    ...
    double turnOutput =
        std::clamp(pidController.Calculate(value, angle), -0.5, 0.5);
...
void WheelDrive::resetEncoder() { m_reverse = false; }

// Returns the velocity of Swerve Module in ft per 0.02s
double WheelDrive::getVelocity() {
    return speedMotor.GetSelectedSensorVelocity() / 2048 * (1 / 6.12) * 4.375 *
           PI / 5 / 12;
}

double WheelDrive::getWheelDistance() {
    return speedMotor.GetSelectedSensorPosition() *
           DriveConstants::EMPIRIC_WHEEL_METERS_PER_TICK;
}

// Returns the angle of Swerve Module, in radians
double WheelDrive::getAngle(double offset) {
    return m_canCoder.GetAbsolutePosition() - offset;
}

double WheelDrive::getAngleRad(double offset) {
    return getAngle(offset) * (M_PI / 180.0);
}

// Initializes the zero position of Swerve Module
void WheelDrive::initialization(double Offset) {
    m_reverse = false;
    // double turnOutput =
    // std::clamp(initializeController.Calculate(getAngle(Offset), Offset),
    // -0.5, 0.5); angleMotor.Set(turnOutput);
}

// Helper function
void WheelDrive::Debug() {
    // frc::SmartDashboard::PutNumber("relative", normalizeEncoderValue());
    // frc::SmartDashboard::PutNumber("absolute", GetabsAngle());
}

void WheelDrive::Stop() {
