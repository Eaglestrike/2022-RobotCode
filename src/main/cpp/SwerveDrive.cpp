...
void
SwerveDrive::TrajectoryFollow(double rot){

    size_t index = m_trajectory_1.getIndex();
    if( abs(GetYPosition() - m_trajectory_1.getY(index)) < 0.1
        && abs(GetXPostion() - m_trajectory_1.getX(index)) < 0.1){
        m_trajectory_1.Progress();
        if(index == m_trajectory_1.getLength() ||
            index > m_trajectory_1.getLength()){
            return;
        }
    }
...
SwerveDrive::GenerateTrajectory_1(){
    ...
    Trajectory::Waypoint p1(10, 3, 0, 90);
    Trajectory::Waypoint p2(1, 2, 0, -90);
    Trajectory::Waypoint p3(5, 6, 0, 0);
