...
void
Shooter::setTargetPos(double position){
    m_targetTurretAng = position;
}
...
Shooter::Aim(){
    ...
    double x_to = (x_off*ShooterConstants::turretMin/360+m_turret.GetSelectedSensorPosition()+ShooterConstants::turretMin);
    x_to = x_to < ShooterConstants::turretMin? x_to:x_to-ShooterConstants::turretMin;
    setTargetPos(x_to);
...
Shooter::peekTurret(double navX, double POV){
    ...
    // m_turret.Set(output);
    setTargetPos(target_turret_tick);
}
