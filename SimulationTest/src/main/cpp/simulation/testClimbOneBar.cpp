...
//     if (abs(ClimbConstants::motorExtendedPose - climber.getMotor().GetSelectedSensorPosition()) <= ClimbConstants::motorPoseTolerance.value()
...
bool testClimbOneBar::retracting() {
    ...
    //  if (abs(ClimbConstants::motorRetractedPose - climber.getMotor().GetSelectedSensorPosition()) <= ClimbConstants::motorPoseTolerance.value()
    //  && climber.getPIDCntrl().AtSetpoint()) {
    //         if (!climber.getBrake().Get()) {
